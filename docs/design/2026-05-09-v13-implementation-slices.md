...
slices never depend on later ones. Slices 2 and 3 are independent after Slice 1
lands and may be reviewed in parallel.
...
6. Optional local runner integration (behind feature flag)
...
## Slice 4: Rate Limits and Safety Gate Extensions
...
## Slice 5: Future GitHub Bridge Threat Model (Docs Only)

**Goal**: Document the future GitHub PR bridge threat model without implementing
code. No CLI stubs, dispatcher action types, or tests land in this slice. The
output is a backlog design that explicitly states GitHub automation is outside
v1.3.
...
| File | Change |
|------|--------|
| `docs/design/2026-05-09-v13-github-bridge.md` | **New file.** Extracted and refined from §5 of the main design doc. Includes threat model, workflow sketch, security requirements, trust boundaries, and what is NOT in v1.3. |

### Tests

No code tests. Documentation review only: verify the bridge document states that
v1.3 ships no GitHub CLI, no `github_pr` dispatcher action, no auto-posting, no
automatic branch/PR creation, and no merges.

### Reviewer prompt

> Verify: (a) no functional GitHub integration code lands, (b) no CLI stubs
> are added, (c) docs explicitly state what is NOT in v1.3, (d) security
> requirements section covers: webhook signatures, actor allowlists, no
> auto-posting, and untrusted PR text handling.
...
local runner subprocess backend with sandbox, timeout, and max_tool_calls via
backend-specific arguments. Include fallback to `argv_fallback`. This slice is optional
...
| File | Change |
|------|--------|
| `a2a_relay/dispatcher.py` | Add `run_agent_runner(action_config, stdin_prompt, *, stdout_max_chars) -> tuple[bool, str, str]` function. Dispatches to a configured local runner subprocess with `shell=False`, timeout, and bounded stdout. On failure, falls back to `argv_fallback` if configured. Update `dispatch_message` to check `action_config.get("type") == "agent_runner"` and route to new function. |
...
1. `test_agent_runner_invokes_subprocess` — mock local runner CLI, verify invocation args.
...
5. `test_agent_runner_fallback_on_failure` — primary runner fails → argv_fallback runs.
...
> backend ran (primary runner vs fallback), (e) existing subprocess tests pass
...
Slice 1 (protocol fields)
    ...
    │
    └── Slice 4 (safety gates) — uses all prior invariants
            │
            ├── Slice 5 (future GitHub bridge docs) — independent, can merge any time after 1
            │
...
- Slice 5: can merge any time after Slice 1 (documentation only; no code dependency).
...
The GPT-5.5 review suggested additional fields such as `causal_chain`,
`root_message_id`, `policy_profile`, and `capabilities_granted`.

- Accepted for v1.3 slices: `auto_generated`, `hop_count`, `max_hops`, and
  metadata-only `body_sha256` in policy events.
- Deferred: `causal_chain` and `root_message_id`; thread and reply fields remain
  sufficient for v1.3 and the richer causal graph can be added after timeline
  consumers need it.
- Deferred: `policy_profile`; current dispatcher policy names and event reasons
  are enough for the first rollout.
- Deferred: `capabilities_granted`; v1.3 treats requested capabilities as
  advisory and does not grant capabilities from message content.
