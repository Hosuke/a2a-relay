...
```json
{
  ...
  "actions": {
    "agent-reply": {
      "type": "agent_runner",
      "runner": "example-agent-runner",
      "model": "example-read-only-model",
...
| `runner` (action)            | string    | Local runner backend identifier; examples use placeholders. |
...
### 5.4 Future CLI Surface (Postponed)

No GitHub CLI commands ship in v1.3. A future bridge design may introduce
human-approved commands for branch creation, review relay, or status sync after
signatures, actor allowlists, replay protection, and public-write approval gates
exist. Until then, GitHub-related thread metadata remains ordinary message body
content, not executable relay behavior.

---

## 6. Multi-Runner Orchestration (Postponed Beyond Draft-Only Runs)
...
- **Review runner** handles review: reading diffs, evaluating correctness,
  safety, and style. It operates in read-only mode with no tool-call write
  permissions.
...
class AgentRunnerAction:
    runner: str          # local backend identifier, e.g. "example-agent-runner"
    model: str           # local model identifier, e.g. "example-read-only-model"
...
Dispatcher invokes an agent runner in sandboxed, read-only mode
...
- Review pass runs and produces structured review.
...
Release scope warning: Milestones 1-2 are v1.3 candidates. Milestone 3 is
optional and feature-flagged; it may ship only after the policy gate and no-loop
invariants have soaked. Milestones 4-6 are design backlog unless each item is
narrowed to private, read-only/draft-only behavior with explicit human approval
for external writes.
...
### Milestone 3: Agent runner action type (optional, feature-flagged)

- Add `AgentRunnerAction` interface in `dispatcher.py`.
- Implement a local `agent_runner` backend as a subprocess with `shell=False`.
...
- Implement `on_reply_before_send` trigger: after a local draft runner produces
  output, an independent local review runner checks it before the reply is sent.
...
- Draft a separate threat model and design for any GitHub bridge.
- Do not add CLI commands or dispatcher action types until that design is approved.
- Future implementation, if any, must be metadata-only by default and require
  explicit human approval for branch creation, PR posting, review relay, and merges.
- Tests: actor allowlists, signature/replay checks, metadata-only dry runs, and
...
- End-to-end test: request → read-only/draft agent output → review/redaction
  gate → private reply.
- Post-v1.3 GitHub/write-flow tests live in a separate backlog design, not in
  v1.3 acceptance.
...
12. Optional: `agent_runner` action type invokes the configured local subprocess.
13. Optional: `sandbox: true` prevents write tool calls.
...
26. Post-v1.3 GitHub/write-flow acceptance tests are intentionally excluded
    from v1.3 and tracked only in a future bridge design.
...
# Example only: kill a systemd-managed watcher if one exists
...
The following event types are v1.3 candidates. GitHub/write-flow event names are
intentionally omitted from v1.3 and should be defined only in a future bridge
threat model.
...
2. **Events**: new event types are additive. Old event readers skip unknown
   types.
3. **CLI**: new `safety-level` and `dispatch --dry-run` surfaces do not conflict
   with existing commands. No `github` CLI is shipped in v1.3.
...
2. **Review model selection**: should review use an independent local runner,
   or the same backend with a different system prompt? Cross-runner review is
   more robust but adds latency and cost.
